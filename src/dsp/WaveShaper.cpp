...
namespace apex {
	namespace dsp {
		namespace waveshapers {
			...
			inline double softSaturation(double input, double amount, double slope) {
				return input * (1 + amount) / (1 + amount * math::pow(math::fabs(input), slope));
			}

			inline float softSaturation(float input, float amount, float slope) {
				return input * (1 + amount) / (1 + amount * math::powf(math::fabsf(input), slope));
			}

			inline double softClip(double input, double amount) {
				return input * (1 + amount) / (1 + amount * math::fabs(input));
			}

			inline float softClip(float input, float amount) {
				return input * (1 + amount) / (1 + amount * math::fabsf(input));
			}

			inline double hardClip(double input, double clipLevel, double amount) {
				double x = (input * (1 + amount) / (1 + amount * math::fabs(input)));
				double signX = x / math::fabs(x);
				return (math::fabs(x) > clipLevel ? clipLevel * signX : x);
			}

			inline float hardClip(float input , float clipLevel, float amount) {
