	...
	template<typename T, typename AttackKind, typename ReleaseKind>
		class DynamicsState {
			public:
				...
				/// @return - The attack
				[[nodiscard]]
				inline auto getAttack() const noexcept -> AttackKind {
					return mAttack;
				}
				...
				/// @return - The release
				[[nodiscard]]
				inline auto getRelease() const noexcept -> ReleaseKind {
					return mRelease;
				}
				...
				/// @return - The ratio
				[[nodiscard]]
				inline auto getRatio() const noexcept -> T {
					return mRatio;
				}
				...
				[[nodiscard]]
				inline auto getThreshold() const noexcept -> T {
					return mThreshold;
				}
				...
				[[nodiscard]]
				inline auto getKneeWidth() const noexcept -> T {
					return mKneeWidth;
				}
				...
				[[nodiscard]]
				inline auto getSampleRate() const noexcept -> size_t {
					return mSampleRate;
				}
				...
				[[nodiscard]]
				inline auto getHasAutoRelease() const noexcept -> bool {
				...
				[[nodiscard]]
				inline auto getAutoReleaseEnabled() const noexcept -> bool {
				...
				[[nodiscard]]
				inline auto getAttackCoefficient1() const noexcept -> T {
				...
				[[nodiscard]]
				inline auto getAttackCoefficient2() const noexcept -> T {
				...
				[[nodiscard]]
				inline auto getAttackCoefficients() const noexcept -> std::tuple<T, T> {
				...
				[[nodiscard]]
				inline auto getReleaseCoefficient1() const noexcept -> T {
				...
				[[nodiscard]]
				inline auto getReleaseCoefficient2() const noexcept -> T {
				...
				[[nodiscard]]
				inline auto getReleaseCoefficients() const noexcept -> std::tuple<T, T> {
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<AttackKind, Field::Attack>(const std::function<void(AttackKind)>& callback) noexcept -> void {
						callback(mAttack);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<ReleaseKind, Field::Release>(const std::function<void(ReleaseKind)>& callback) noexcept -> void{
						callback(mRelease);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<T, Field::Ratio>(const std::function<void(T)>& callback) noexcept -> void {
						callback(mRatio);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<T, Field::Threshold>(const std::function<void(T)>& callback) noexcept -> void {
						callback(mThreshold);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<T, Field::KneeWidth>(const std::function<void(T)>& callback) noexcept -> void {
						callback(mKneeWidth);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<size_t, Field::SampleRate>(const std::function<void(size_t)>& callback) noexcept -> void {
						callback(mSampleRate);
				...
				/// The callback is called immediately to allow for synchronization with the current state.
				template<>
					auto registerCallback<bool, Field::AutoRelease>(const std::function<void(bool)>& callback) noexcept -> void {
						callback(mAutoReleaseEnabled);
						mAutoReleaseCallbacks.push_back(callback);
					}

				auto operator=(DynamicsState<T, AttackKind, ReleaseKind>&& state)
					noexcept -> DynamicsState<T, AttackKind, ReleaseKind>& = default;
