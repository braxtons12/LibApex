	...
	class Gain : public Processor<T> {
	  public:
		static_assert(
			...
			"Gain must be instantiated with a floating point template parameter (float or double)");

	  private:
		JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(Gain)
	};

	/// @brief Class used for managing, storing, and applying a specifc gain; operating with
	/// `float`s
	template<>
