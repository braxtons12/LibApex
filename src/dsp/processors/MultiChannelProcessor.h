...
namespace apex::dsp {
	/// @brief Multichannel `Processor` class to duplicate a single processor type across multiple
	/// channels
	///
	...
	class MultiChannelProcessor<float, F> {
	  public:
		...
		[[nodiscard]] auto getNumChannels() const noexcept -> size_t {
	...
	class MultiChannelProcessor<double, F> {
	  public:
		...
		using MultiChannelArray
			= gsl::span<gsl::span<double, gsl::dynamic_extent>, gsl::dynamic_extent>;
