	...
	template<>
		class ParallelEQBand<float> : public EQBand<float> {
			public:
				...
				ParallelEQBand(float frequency, float q, float gainDB,
				...
				[[nodiscard]]
					auto getGainDB() const noexcept -> float override;
				...
				[[nodiscard]]
					auto getMagnitudeForFrequency(float frequency) const noexcept -> float override;
				...
				/// @return - The magnitude response at the given frequency
				[[nodiscard]]
					auto getDecibelMagnitudeForFrequency(float frequency)
				...
				[[nodiscard]]
					auto getPhaseForFrequency(float frequency) const noexcept -> float override;
				...
				[[nodiscard]]
					auto getDegreesPhaseForFrequency(float frequency) const noexcept -> float override;
				...
				auto getDegreesPhasesForFrequencies(
						gsl::span<float, gsl::dynamic_extent> frequencies,
						gsl::span<float, gsl::dynamic_extent> phases)
					const noexcept -> void override;

				auto operator=(ParallelEQBand<float>&& band)
					noexcept -> ParallelEQBand<float>& = default;
	...
	template<>
		class ParallelEQBand<double> : public EQBand<double> {
			public:
				...
				/// @param sampleRate - The sample rate to use, in Hertz
				/// @param type - The band type to use
				ParallelEQBand(double frequency, double q, double gainDB,
						size_t sampleRate, BandType type)
					noexcept;
				...
				/// @return - The current gain, in Decibels
				[[nodiscard]]
					auto getGainDB() const noexcept -> double override;
				...
				/// @return - The magnitude response at the given frequency
				[[nodiscard]]
					auto getMagnitudeForFrequency(double frequency) const noexcept -> double override;
				...
				/// @return - The magnitude response at the given frequency
				[[nodiscard]]
					auto getDecibelMagnitudeForFrequency(double frequency)
				...
				/// @return - The phase response, in radians, at the given frequency
				[[nodiscard]]
					auto getPhaseForFrequency(double frequency) const noexcept -> double override;
				...
				/// @return - The phase response, in degrees, at the given frequency
				[[nodiscard]]
					auto getDegreesPhaseForFrequency(double frequency)
						const noexcept -> double override;
				...
				auto operator=(ParallelEQBand<double>&& band)
					noexcept -> ParallelEQBand<double>& = default;
