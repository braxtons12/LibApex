...
namespace apex::dsp {
	...
	EQBand<float>::EQBand(float frequency, float q, float gainDB,
	...
	///
	/// @return - The magnitude response at the given frequency
	inline auto EQBand<float>::getDecibelMagnitudeForFrequency(float frequency)
	...
	inline auto EQBand<float>::getDegreesPhaseForFrequency(float frequency)
		const noexcept -> float
		{
	...
	/// @param sampleRate - The sample rate to use, in Hertz
	/// @param type - The type of band
	EQBand<double>::EQBand(double frequency, double q, double gainDB,
			size_t sampleRate, BandType type)
		noexcept
		: mType(type),
		mFrequency(frequency),
	...
	///
	/// @return - The magnitude response at the given frequency
	inline auto EQBand<double>::getMagnitudeForFrequency(double frequency)
	...
	///
	/// @return - The magnitude response at the given frequency
	inline auto EQBand<double>::getDecibelMagnitudeForFrequency(double frequency)
		const noexcept -> double
		{
			return math::Decibels::linearToDecibels(getMagnitudeForFrequency(frequency));
		}
	...
	///
	/// @return - The phase response, in degrees, at the given frequency
	inline auto EQBand<double>::getDegreesPhaseForFrequency(double frequency)
		const noexcept -> double
		{
