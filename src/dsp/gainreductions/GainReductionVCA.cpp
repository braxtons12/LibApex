...
namespace apex {
	namespace dsp {
		...
		GainReductionVCA<float>::GainReductionVCA(size_t sampleRate)
		...
		void GainReductionVCA<float>::reset(float currentGainReduction) {
			mCurrentSample = 0;
		...
		float GainReductionVCA<float>::adjustedGainReduction(float actualGainReduction, float idealGainReduction) {
		...
		void GainReductionVCA<float>::setSampleRate(size_t sampleRate) {
		...
		///
		/// @param sampleRate - The sample rate to use
		GainReductionVCA<double>::GainReductionVCA(size_t sampleRate)
		...
		///
		/// @param currentGainReduction - The gain reduction to use as the initial value
		void GainReductionVCA<double>::reset(double currentGainReduction) {
			mCurrentSample = 0;
			mCurrentGainReduction = currentGainReduction;
		}
		...
		///
		/// @return - The adjusted gain reduction
		double GainReductionVCA<double>::adjustedGainReduction(double actualGainReduction, double idealGainReduction) {
		...
		///
		/// @param sampleRate - The new sample rate to use
		void GainReductionVCA<double>::setSampleRate(size_t sampleRate) {
			mSampleRate = sampleRate;
			mNumSamplesToTransitionGain = static_cast<size_t>(mRiseTimeSeconds * mSampleRate + 0.5);
		}
	}
