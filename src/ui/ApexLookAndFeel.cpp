...
namespace apex {
	namespace ui {
		...
		void ApexLookAndFeel::drawRotaryApexSlider(juce::Graphics& g,
												   int x,
												   int y,
												   int width,
												   int height,
		...
		void ApexLookAndFeel::drawLinearSliderBackground(juce::Graphics& g,
														 int x,
														 int y,
														 int width,
														 int height,
		...
		void ApexLookAndFeel::drawLinearApexSliderBackground(juce::Graphics& g,
															 int x,
															 int y,
															 int width,
															 int height,
			...
			float sliderMinPos = static_cast<float>(
		...
		void ApexLookAndFeel::drawLinearSliderThumb(juce::Graphics& g,
													int x,
													int y,
													int width,
													int height,
		...
		void ApexLookAndFeel::drawLinearApexSliderThumb(juce::Graphics& g,
														int x,
														int y,
														int width,
														int height,
		...
		void ApexLookAndFeel::drawLinearSlider(juce::Graphics& g,
											   int x,
											   int y,
											   int width,
											   int height,
		...
		void ApexLookAndFeel::drawLinearApexSlider(juce::Graphics& g,
												   int x,
												   int y,
												   int width,
												   int height,
			...
			if(maybeFilmStrip.isSome()) {
				...
				float sliderMaxPos = static_cast<float>(
		...
		void ApexLookAndFeel::drawPopupMenuItem(juce::Graphics& g,
		...
		void ApexLookAndFeel::drawComboBox(juce::Graphics& g,
										   int width,
										   int height,
										   bool isButtonDown,
										   int buttonX,
			...
			juce::Rectangle<int> arrowZone(static_cast<int>(width * 0.75f),
										   0,
		...
		void ApexLookAndFeel::drawApexComboBox(juce::Graphics& g,
											   int width,
											   int height,
											   bool isButtonDown,
											   int buttonX,
			...
			if(maybeActiveArrow.isSome()) {
				...
				juce::Rectangle<float> boxBounds
					= juce::Rectangle<int>(0, 0, width, height).toFloat();
				juce::Rectangle<float> troughBounds = boxBounds.reduced(2.0f, 2.0f);
				juce::Rectangle<float> buttonBounds = troughBounds.reduced(2.0f, 2.0f);
				juce::Rectangle<float> shadowBounds = troughBounds.expanded(2.0f, 2.0f);
				juce::Rectangle<int> arrowZone(static_cast<int>(width * 0.75f),
				...
				juce::Colour textColour
					= (box.isPopupActive() ?
							 mComboBoxTextColour.brighter(0.2f) :
							 (box.isHovered() ?
								  mComboBoxTextColour.brighter(0.1f) :
								  (box.isEnabled() ? mComboBoxTextColour :
													 mComboBoxTextColour.darker(0.3f))));
				g.setColour(textColour);
				g.strokePath(p, juce::PathStrokeType(2.0f));
		...
		void ApexLookAndFeel::drawComboBoxTextWhenNothingSelected(juce::Graphics& g,
																  juce::ComboBox& box,
																  juce::Label& label) {
			juce::Colour textColour
				= (box.isPopupActive() ?
		...
		void ApexLookAndFeel::drawToggleButton(juce::Graphics& g,
											   juce::ToggleButton& button,
											   bool shouldDrawButtonAsHighlighted,
											   bool shouldDrawButtonAsDown) {
			float cornerSize
				= juce::jmax(5.0f, button.getWidth() * 0.1f, button.getHeight() * 0.1f);
		...
		void ApexLookAndFeel::drawApexToggleButton(juce::Graphics& g,
		...
		void ApexLookAndFeel::drawApexMeter(juce::Graphics& g,
			...
			if(maybeFilmStrip.isSome()) {
				...
				int meterLevelY
					= static_cast<int>(meterY + (1.0f - levelProportional) * meterHeight);
		...
		juce::Rectangle<int>
		ApexLookAndFeel::getActualRotaryBounds(int x, int y, int width, int height) {
#if defined USE_PHYSICAL_ROTARIES
			float diameter = static_cast<float>(juce::jmin(width, height));
			juce::Rectangle<float> bounds(x + (width - diameter) / 2.0f,
		...
		void ApexLookAndFeel::drawPhysicalRotary(juce::Graphics& g,
												 int x,
												 int y,
												 int width,
												 int height,
		...
		void ApexLookAndFeel::drawSemiCircularRotary(juce::Graphics& g,
													 int x,
													 int y,
													 int width,
													 int height,
		...
		void ApexLookAndFeel::drawCircularFillRotary(juce::Graphics& g,
													 int x,
													 int y,
													 int width,
													 int height,
