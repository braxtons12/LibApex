...
namespace apex {
	namespace math {
		...
		constexpr inline float cos_helperf(float x) noexcept {
		...
		constexpr inline float cosf(float angle) noexcept {
		...
		constexpr inline float sinf(float angle) noexcept {
			return cosf(piOver2f - angle);
		}

		constexpr inline float tan_helperf(float x) noexcept {
		...
		constexpr inline float tanf(float angle) noexcept {
			angle = fmodf(angle, twoPif);				// Get rid of values >2 *pi
		...
		constexpr inline float atan_helperf(float x) noexcept {
		...
		constexpr inline float atanf(float angle) noexcept {
		...
		constexpr inline float tanhf(float angle) noexcept {
			float angle2 = angle * angle;
			return angle * ( 27.0f + angle2) / (27.0f + 9.0f * angle2);
		}

		constexpr inline double cos_helper(double x) noexcept {
		...
		constexpr inline double cos(double angle) noexcept {
		...
		constexpr inline double sin(double angle) noexcept {
			return cos(piOver2 - angle);
		}

		constexpr inline double tan_helper(double x) noexcept {
		...
		constexpr inline double tan(double angle) noexcept {
			angle = fmod(angle, twoPi);				// Get rid of values >2 *pi
		...
		constexpr inline double atan_helper(double x) noexcept {
		...
		constexpr inline double atan(double angle) noexcept {
			bool complement = false;				// true if arg was >1
			bool region = false;					// true depending on region arg is in
			bool sign = false;					// true if arg was < 0
		...
		constexpr inline double tanh(double angle) noexcept {
